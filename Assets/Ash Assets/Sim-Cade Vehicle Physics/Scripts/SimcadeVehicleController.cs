...
namespace Ashsvp
{
    public class SimcadeVehicleController : MonoBehaviour
    {
        ...
        private void Start()
        {
            input = FindAnyObjectByType<LevelManager>().Input;
            VehicleEvents.OnGrounded.AddListener(ReadyToReset);
        ...
        private void Update()
        {
            if (CanDrive && CanAccelerate)
            {
                ...
                brakeInput = input.Player.Jump.ReadValue<float>();
            }
            else if(CanDrive && !CanAccelerate)
            {
                accelerationInput = 0;
                steerInput =input.Player.Move.ReadValue<Vector2>().x;
                brakeInput = input.Player.Jump.ReadValue<float>();
            }
            else
            {
            ...
            if (isReadyToReset && input.Player.Reset.IsPressed())
            {
                ResetVehicle();
            }
        }
        ...
        {
            VehicleEvents.OnGrounded.RemoveListener(ReadyToReset);
        }

        public void ResetVehicle(Transform point = null)
        {
            if(rb is null)
                return;
